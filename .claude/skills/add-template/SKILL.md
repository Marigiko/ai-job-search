...
name: add-template
description: >
  Register a custom CV or cover letter LaTeX template with the framework — store the skeleton, capture
  compile/style rules, run a mandatory test compile, and activate via a managed block in the guidance
  file so the apply skill uses it. Supports --list and --use subcommands.
  Triggers on: /add-template, add template, register template, custom template, use template, list templates
allowed-tools: Read, Write, Edit, Glob, Grep, Bash(lualatex *), Bash(xelatex *), Bash(pdflatex *)
---
...
- If the user's message contains `--list`: run **List Mode** below and stop.
- If the user's message contains `--use <name>`: run **Switch Mode** below, then continue to **Step 5: Activate** with the resolved template metadata. `--use default` deactivates any custom template and restores the stock guidance (see Step 5).
- If the user's message contains a file path or @-mentioned file: treat it as the template source and carry it into Step 1.
...
If the user's message contains `--use <name>`:
...
Ask the user (skip anything already answered by their message):
...
5. **Page limit** - hard page count for the compiled PDF. Default: **2 pages** for a CV, **1 page** for a cover letter. The `apply` skill's compile-and-inspect loop enforces this.
...
Activation wires the template into `apply` by adding a **managed block** to the top of the relevant guidance file — `05-cv-templates.md` for CVs, `06-cover-letter-templates.md` for cover letters. The `apply` skill reads these files in its drafting step, so the block is all it takes.
...
> - The `apply` skill will now draft <CVs | cover letters> from this template.
>
> Useful follow-ups:
