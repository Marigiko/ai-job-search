...
name: reset
description: >
  Reset parts of the job search framework back to a blank state — clears profile data from skill files
  and/or deletes user-provided documents. Destructive: requires explicit RESET confirmation before
  any file is touched. Triggers on: /reset, reset profile, reset documents, start over, reset all, clear profile
allowed-tools: Read, Write, Edit, Glob, Bash(rm *)
---
...
You are resetting parts of the job search framework back to a blank state so the user can start fresh with the `setup` skill.

**This skill is destructive.** Nothing is deleted until the user explicitly confirms. Follow these steps exactly in order.

---

## Step 0: Parse Scope from Arguments

Check the user's message for a scope keyword:
...
If the user's message is empty or does not contain a recognized scope keyword, ask:

> **What would you like to reset?**
>
> - **`profile`** — Clears candidate data from the skill files (profile, behavioral, STAR examples, profile statements). The framework structure and writing rules are preserved. Use this to re-run the `setup` skill from scratch.
>
...
<!-- Run the setup skill to populate this file -->
...
<!-- Run the setup skill to populate role-specific profile statements -->
```
...
<!-- Run the setup skill to populate STAR examples from your actual experience -->
```
...
> Your candidate profile is now blank. Run the `setup` skill to repopulate it. It auto-detects any files in your `documents/` folder and offers to read from there; otherwise it walks you through a CV import or interactive interview.

**If documents were reset:**
> The `documents/` folder is now empty. Add your career documents and run the `setup` skill to populate your profile. See `documents/README.md` for instructions on what to put where.

**If both were reset:**
> Both your profile files and documents folder are now empty. Add documents to `documents/` (or skip and use the CV import / interview path), then run the `setup` skill.
