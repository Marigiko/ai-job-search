...
name: expand
description: >
  Enrich the candidate profile by discovering competencies hidden in documents and public online
  presence (GitHub, LinkedIn, diplomas, references). Additive only — never modifies existing content.
  Triggers on: /expand, expand profile, enrich profile, discover competencies, scan profile, expand skills
...
You are enriching the candidate profile by discovering competencies hidden in documents and public online presence. This skill is additive only — it never modifies existing profile content, only extends it.
...
Hold this content in context throughout the skill. Do not re-read these files later.
...
- **Additive only.** This skill never modifies existing profile content. It only appends.
