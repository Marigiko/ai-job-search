...
1. Read `job_search_tracker.csv`. Extract all rows. The columns are:
   ...
   (Older trackers may have the shorter 13-column header without `salary_expected`/`salary_offered`/`relocation_visa`/`application_url` — read by column name and tolerate their absence.)
