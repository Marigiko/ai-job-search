...
name: outcome
description: >
  Record the result of a job application - progress updates (interview invitations, stages completed, offers)
  and final resolutions (hired, rejected, no response). Updates the tracker CSV and archives materials per
  application. Triggers on: /outcome, outcome, record outcome, interview result, application status, rejected, hired
...
- `job_search_tracker.csv` - the status column that the `scrape` skill and `rank` skill use for dedup and exclusion
- `documents/applications/<company>_<role>/` - the per-application archive (posting, submitted drafts, `outcome.md`) that `setup` skill Path A mines to calibrate `04-job-evaluation.md` and surface STAR candidates

The `outcome` skill writes the data; the `setup` skill interprets it. This skill never edits the evaluation framework or profile files itself.
...
1. Read `job_search_tracker.csv`. If it does not exist, create it with the standard header:
   ```
   date,company,sector,role,role_type,channel,status,contact_person,fit_rating,salary_expected,salary_offered,relocation_visa,application_url,notes,cv_file,cover_letter_file,source
   ```
   Populate the extended columns when known (leave blank otherwise): `salary_expected` (candidate's target from CLAUDE.md → Compensation), `salary_offered` (what the posting/employer states), `relocation_visa` (one of `sponsor` / `relocation` / `remote` / `none` / `unknown`), `application_url` (the actual apply link or `mailto:` — distinct from `source`, the posting URL). If an older tracker with the 13-column header is found, keep its rows and append the new columns to the header on next write.
...
3. **`outcome.md`** - write or update it in exactly the format documented in `documents/README.md`, so the `setup` skill Path A parses it without special cases:
...
- If 3 or more are resolved (or 2+ share a pattern - same role type rejected twice, same sector going silent), suggest:
  > "You now have <N> resolved applications on record. Run the `setup` skill (Path A) to fold them into your evaluation framework - it calibrates fit scoring from what actually got interviews, and mines your interview feedback for STAR examples."
- Do **not** write anything into `04-job-evaluation.md` or other skill files yourself. The `setup` skill Path A owns that merge - it is read-before-write and idempotent, and duplicating its logic here would race it.
...
> "Interview coming up? Run the `interview` skill for that company — it builds a prep pack for that stage from this application's archive - the posting, the documents you submitted, and any feedback recorded from earlier rounds."
...
1. **Write data, don't interpret it.** The archive and tracker are the outputs; calibration belongs to `setup`. This skill never edits profile or framework files.
