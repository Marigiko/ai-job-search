...
name: add-portal
description: >
  Generate a job-portal search skill for a job board in any market — investigate the portal, scaffold the
  CLI skill from the canonical linkedin-search structure, test-run a live query, and register it in
  .agents/skills/. Triggers on: /add-portal, add portal, new portal skill, generate portal, register portal
allowed-tools: Read, Write, Edit, Glob, Grep, Bash(bun *), WebFetch
---
...
You are helping the user build a job-portal search skill for a job board in their market. The repo ships worked examples of the pattern (four Danish portals plus the country-agnostic `linkedin-search`), and the README invites users elsewhere to build equivalents — this skill turns that invitation into a guided workflow: investigate the portal, scaffold the skill from the canonical structure, and test-run a live query before registering anything.
...
The user's message may contain a subcommand, a portal URL, or nothing.
...
- If the user's message contains `--list`: use Glob with `.agents/skills/*/SKILL.md`, print a table of installed portal skills (name, market from the description, data source from `url-reference.md`), and stop.
- If the user's message contains a URL: treat it as the portal URL and carry it into Step 1.
...
These conventions are what make portal skills interchangeable for the `scrape` skill and for users reading any skill's docs:
...
1. Ask whether the user wants the new portal added to their `scrape` skill search strategy. If yes, add the portal's site to the relevant query categories in `.claude/skills/job-scraper/search-queries.md` (site-specific queries, like the existing `jobindex.dk` entries) so `scrape` includes it.
...
- Investigation before scaffolding: the skill never generates parsers from guesses - Step 2 fetches real responses first, and Step 4 verifies against live data before anything is registered.
