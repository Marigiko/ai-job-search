#!/usr/bin/env bun
// Self-contained CLI for the Remotive public API (JSON, no auth, zero runtime
// dependencies). Remote-only board with server-side keyword search. Mirrors the
// repo's portal-skill contract.
...
const HELP = `remotive-cli — search remote jobs on the Remotive public API (all remote)
...
SEARCH FLAGS
  ...
  --location, -l <text>   Filter by candidate-location substring (client-side). Optional.
...
EXAMPLES
  ...
  bun run src/cli.ts search -q python -l "Europe" --format table
  bun run src/cli.ts detail 2091062 --format plain

Public API. Errors -> stderr { "error", "code" }, exit 1.
`
...
async function main(): Promise<number> {
  ...
  if (cmd === "search") {
    ...
    for (const key of ["jobage", "limit"] as const) {
